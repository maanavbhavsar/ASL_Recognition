...
###
## Mode - RGB or optical flow
## Root - Directory where videos are stored
## train_split - tells us how to split up the video into train/test
## save_model - directory where to save the checkpoints
...
def run(mode='rgb',
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=16, shuffle=True, num_workers=0,
                                             pin_memory=True) #configs.batch_size
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=16, shuffle=True, num_workers=0,
                                                 pin_memory=False) #configs.batch_size
    dataloaders = {'train': dataloader, 'test': val_dataloader} #### what is this dic doing?
    ...
    lr = 0.0001 #configs.init_lr
    weight_decay = 1e-8 #configs.adam_weight_decay
    optimizer = optim.Adam(i3d.parameters(), lr=lr, weight_decay=weight_decay)

    # Should just be 1
    num_steps_per_update = 1 #configs.update_per_step  # accum gradient
    steps = 0
    epoch = 0
    ...
    num_epochs = 400
    while steps < 64000 and epoch < num_epochs:  # for epoch in range(num_epochs): configs.max_steps
        ...
        for phase in ['train', 'test']:
            ...
            for data in dataloaders[phase]:
                ...
                for i in range(per_frame_logits.shape[0]):
                    confusion_matrix[gt[i], torch.argmax(predictions[i]).item()] += 1
                ...
                if num_iter == num_steps_per_update and phase == 'train':
                    ...
                    if steps % 10 == 0:
                        ...
                        print('Step {}/{} within epoch - max steps: {}'.format(steps, num_steps_loader, 64000))
                        print(
